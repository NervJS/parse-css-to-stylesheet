...
use crate::{generate_expr_by_length, generate_expr_enum, generate_expr_lit_str, generate_expr_lit_num};
...
impl StyleMedia {
    pub fn new(media_id:u32, conditions: Vec<StyleMediaCondition>) -> Self {
        StyleMedia { media_id, conditions}
    }
        ...
        return vec![
            ...
            PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
                ...
                value: Box::new(Expr::Array(ArrayLit {
                    span: DUMMY_SP,
                    elems: self.conditions.iter().map(|condition|{self.condtition_to_expr(condition)}).collect()
                })),
            })))
        ];
    }
    ...
    fn condtition_to_expr(& self, condtion: & StyleMediaCondition ) -> Option<ExprOrSpread>{
        match condtion {
            ...
            StyleMediaCondition::Operation { operation, conditions } => {
                let condition_arr: Vec<Option<ExprOrSpread>> = conditions.iter().map(|elem|{
                    self.condtition_to_expr(elem)
                }).collect();
                return Some(ExprOrSpread{
                    spread: None,
                    expr: Box::new(Expr::Array(ArrayLit{
    ...
    fn parse_condition(& mut self, condition: &MediaCondition)->Option<StyleMediaCondition> {
        match condition {
            MediaCondition::Feature(feature) => {
                match feature {
                    ...
                    QueryFeature::Interval { name, start, start_operator, end, end_operator } => {
                        ...
                        return Some(StyleMediaCondition::Operation { operation: StyleMediaCondType::AND, conditions: vec![
                            self.parse_feature(name, start_op, Some(start)),
                            self.parse_feature(name, end_op, Some(end))
                        ].into_iter().filter_map(|x|{x.map(|y| StyleMediaCondition::Feature(y))}).collect()});
                    },
                }
            },
            MediaCondition::Not(not) => {
                ...
                match ret_condition {
                    Some(ret_some) => { 
                        return Some(StyleMediaCondition::Operation{operation:StyleMediaCondType::NOT, conditions:vec![ret_some]});},
                    None => {return None;},
                }
            },
            MediaCondition::Operation { operator, conditions } => {
                ...
                return Some(StyleMediaCondition::Operation{operation:op, conditions:ret_conditions});
            },
        }
    }

    fn parse_feature(&mut self, feature_id: &MediaFeatureName<MediaFeatureId>, feature_op: StyleMediaOpType, feature_val: Option<&MediaFeatureValue>)->Option<StyleMediaFeature> {
        let id = self.convert_featureid(feature_id);
        return match feature_val {
            Some(value) => Some(StyleMediaFeature{feature_id:id, op: feature_op, value:self.parse_value(value)}),
            None => Some(StyleMediaFeature{feature_id:id, op: feature_op, value: None}),
        };
    }
    ...
    fn parse_value(&mut self, feature_val: &MediaFeatureValue) -> Option<MediaValueType>{
        match feature_val {
            ...
            MediaFeatureValue::Env(_) => {
                return None
            }
        }
    }

    fn convert_featureid(&mut self, feature_id: &MediaFeatureName<MediaFeatureId>)->StyleMediaFeatureID{
        match feature_id {
            MediaFeatureName::Standard(stand_id) => {
                match stand_id {
