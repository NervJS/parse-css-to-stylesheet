...
use lightningcss::{printer::PrinterOptions, properties::{transition, Property, PropertyId}, traits::ToCss, values::{easing::EasingFunction,  time}};

use crate::{generate_expr_lit_num, generate_expr_enum, generate_expr_lit_str, style_propetries::style_property_enum, visitor::parse_style_values};
use swc_core::{common::DUMMY_SP, ecma::ast::*};
use super::{style_property_type::CSSPropertyType, style_value_type::StyleValueType, traits::ToExpr, unit::{Platform, PropertyTuple}};
  ...
  fn from(value: (String, &Property<'_>)) -> Self {
    ...
    match value.1 {
      ...
      Property::TransitionTimingFunction(timing_function, _) => {
        timing_function.into_iter().for_each(|transition_timing_function| {
...
impl ToExpr for Transition {
  fn to_expr(&self) -> PropertyTuple {
    ...
    if !self.transtition_properties.is_empty() {
      ...
      let array_elements: Vec<_> = properties.into_iter().map(|property| {
    ...
    if !self.transition_delays.is_empty() {
      ...
      let array_elements: Vec<_> = delays.into_iter().map(|delay| {
        // Assuming `generate_expr_lit_num!` generates a numeric expression
    ...
    if !self.transition_timeing_functions.is_empty() {
      let array_elements: Vec<_> = self.transition_timeing_functions.iter().map(|timing_function| {
