...
pub struct OutputOptions {
  ...
  // flatbuffer 产物版本: 不传为旧版(v1)，传 "v2" 走带字符串池的新版 schema
...
pub fn parse(styles: Vec<String>, options: ParseOptions) -> ParseResult {
  ...
  let is_v2 = output.version.as_deref() == Some("v2");
  ...
  if is_bin {
    // 输出到文件，根据 version 选择新旧 flatbuffer schema
    let convert_result = if is_v2 {
      flatbuffer_v2::convert_json_to_flatbuffer(&style_json)
    } else {
      utils::convert_json_to_flatbuffer(&style_json)
    };
...
#[cfg(test)]
mod tests {
  use crate::flatbuffer_v2::convert_json_to_flatbuffer as convert_json_to_flatbuffer_v2;
  ...
  fn test_valid_input_v2() {
    ...
    let result = convert_json_to_flatbuffer_v2(&json_input);
    assert!(result.is_ok());

    let buffer = result.unwrap();
    let style_sheet = styles_v2::root_as_style_sheet(&buffer).unwrap();
    let string_pool = style_sheet.string_pool().unwrap();
    ...
    assert_eq!(string_pool.get(first_variable.key() as usize), "--color");
    assert_eq!(string_pool.get(first_variable.value() as usize), "red");
    ...
    assert!(third_declaration.value_as_pool_string().is_some());
    assert_eq!(string_pool.get(third_declaration.value_as_pool_string().unwrap().id() as usize), "var(--h)");
    assert_eq!(third_declaration.flag(), 1);

    let first_selector = first_style.selector().unwrap().get(0);
    assert!(first_selector.is_string());
    assert_eq!(string_pool.get(first_selector.string_id() as usize), "app");
    ...
    assert!(first_selector.is_string());
    assert_eq!(string_pool.get(first_selector.string_id() as usize), "tit");
    let second_selector = selector.get(1);
    assert!(!second_selector.is_string());
    ...
    let first_selector = selector.get(0);
    assert_eq!(string_pool.get(first_selector.string_id() as usize), "img");
    ...
    assert_eq!(selector.len(), 1);
    let first_selector = selector.get(0);
    assert_eq!(string_pool.get(first_selector.string_id() as usize), "hello");
    ...
    assert_eq!(selector.len(), 1);
    let first_selector = selector.get(0);
    assert_eq!(string_pool.get(first_selector.string_id() as usize), "bbb");
